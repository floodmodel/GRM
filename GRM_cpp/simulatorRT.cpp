...
int startSimulationRT()
{
    ...
    while (nowTsec < endingT_sec) {
        ...
        if (curRForder == 0 || nowTsec > dtRF_sec * ts.rfDataCountTotal) {
            ...
            int rft_sec = curRForder * dtRF_sec;
            ...
            while (ts.stopSim == -1) { 
                updateRFinfoGRMRT(targetRFLayerTime);
                if (curRForder < ts.rfDataCountTotal) { break; }
            ...
            for (int idx : fccds.cvidxsFCcell) {
                ...
                if (afc.fcType != flowControlType::ReservoirOperation) {
                    ...
                    if (nowTsec > dt_min * 60 * fccds.curDorder[idx]
                        ...
                        while (ts.stopSim == -1) {
                            if (targetDataTime_Previous != targetDataTime 
                                | bAfterSleep==true) {
                                ...
                                if (ts.enforceFCautoROM == -1 && tsrt.newFcDataAddedRT[idx] == 1) { break; }//2019.10.01. 최. prediction 관련
                                ...
                                // 데이터로 모델링 하던 것은 모두 ReservoirOperation,  AutoROM으로 변경됨
                                 // 즉, fc 데이터가 있는 것 까지는 데이터 이용, 없으면, autoROM
                                    mRealTime.ConvertFCinfoToAutoROM(targetDataTime, cvidx);
                                    break;
                                }
                                _sleep(2000);
                                bAfterSleep = true;
                            }
                            mRealTime.mdicFCDataOrder[cvidx] += 1;
                        }
                        if (ts.stopSim == 1) {
                            writeLog(fpnLog, "Real time simulation was stopped.\n", 1, 1);
                            return 1;
