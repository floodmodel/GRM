...
#pragma omp parallel for
 ...
 int setLAIRatio()
 {
	 if (prj.fpnLAI != "" && fs::exists(lower(prj.fpnLAI)) != true) {//MP 수정 _access 대신 fs::exists 사용
...
#pragma omp parallel for schedule(guided)
 ...
 int setSunDurationRatioData()// 내장하는 것으로 수정. 2025.01.24
 {
	 ...
	 //if (prj.fpnDaytimeLengthRatioData != "" 
		// && fs::exists(lower(prj.fpnDaytimeLengthRatioData)) != true) {//MP 수정 _access 대신 fs::exists 사용
		// writeLogString(fpnLog, "The ratio of daytime hours data file is invalid.\n", 1, 1);
		// return -1;
	 //}
	 //memset(sunDurRatioForAday, 0, 12 * sizeof(sunDurRatioForAday[0])); // 12 개의 자료가 있으므로,,
	 //vector<double> Lines;
	 ...
	 //if (Lines.size() > 12) {
		...
		// return -1;
	 //}
	 //copy(Lines.begin(), Lines.end(), sunDurRatioForAday);
	 int xRow_Lat = 65;
	 int yCol_Month = 13;
 ...
 int setBlaneyCriddleK() {
	 ...
	 if (prj.fpnBlaneyCriddleK != "" // Project open에서 검증했는데, 여기서 또한번 검증
		 && fs::exists(lower(prj.fpnBlaneyCriddleK)) != true) {//MP 수정 _access 대신 fs::exists 사용
 ...
 vector<weatherData> readAndSetWeatherData(string fpn_in_wd, weatherDataType wdType,
	 ...
	 if (fpn_in_wd != "" 
		 && fs::exists(lower(fpn_in_wd)) != true) {//MP 수정 _access 대신 fs::exists 사용
 ...
 //  이 부분 하나의 함수로 처리하기 위해서 주석처리. 개별함수로 분리하는 효과 없다. 2024.12.11
     // int setCVtempMax(int order)
	 // {
	 //	 string fpnData = "";
	 //	 int returnv = 1;
	 //	 fpnData = tempMax[order - 1].FilePath + "/" + tempMax[order - 1].FileName;
	 //	 switch (prj.tempMaxDataType)
	 //	 {
	 //	 case weatherDataType::Raster_ASC: {
	 //		 ascRasterFile* wdAsc;
	 //		 wdAsc = new ascRasterFile(fpnData);
	 //		 if (compareASCwithDomain(fpnData, "max. temperature", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			 return -1;
	 //		 }
	 //		 omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	 }
	 //	 case weatherDataType::Mean: {
	 //		 double wdValue = stod(tempMax[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		 for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			 // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			 // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	 }
	 //	 if (returnv == -1) {
	 //		 writeLogString(fpnLog, "ERROR : An error was occurred while reading max. temperature data.\n", 1, 1);
	 //		 writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	 }
	 //	 return returnv;
	 // }
	 ...
	 //	  string fpnData = "";
	 //	  int returnv = 1;
	 //	  fpnData = tempMin[order - 1].FilePath + "/" + tempMin[order - 1].FileName;
	 //	  switch (prj.tempMinDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "min. temperature", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(tempMin[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading min. temperature data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  switch (prj.DTLDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "daytime length", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(dayTimeLength[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading daytime length data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  switch (prj.solarRadDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "solar radiation", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(solarRad[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading solar radiation data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  string fpnData = "";
	 //	  int returnv = 1;
	 //	  fpnData = dewPointTemp[order - 1].FilePath + "/" + dewPointTemp[order - 1].FileName;
	 //	  switch (prj.dewPointTempDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "dew point temperature", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  case weatherDataType::Mean_DividedArea: {
	 //		  map<int, double> idNv;
	 //		  idNv = dewPointTemp[order - 1].vForEachRegion;
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  double wdValue = idNv[cvps[i].wsid];
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(dewPointTemp[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading dew point temperature data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  string fpnData = "";
	 //	  int returnv = 1;
	 //	  fpnData = windSpeed[order - 1].FilePath + "/" + windSpeed[order - 1].FileName;
	 //	  switch (prj.windSpeedDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "wind speed", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  case weatherDataType::Mean_DividedArea: {
	 //		  map<int, double> idNv;
	 //		  idNv = windSpeed[order - 1].vForEachRegion;
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  double wdValue = idNv[cvps[i].wsid];
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(windSpeed[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading wind speed data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  string fpnData = "";
	 //	  int returnv = 1;
	 //	  fpnData = userPET[order - 1].FilePath + "/" + userPET[order - 1].FileName;
	 //	  switch (prj.userPETDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "user defined potential evapotranspiration", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  case weatherDataType::Mean_DividedArea: {
	 //		  map<int, double> idNv;
	 //		  idNv = userPET[order - 1].vForEachRegion;
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  double wdValue = idNv[cvps[i].wsid];
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(userPET[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  default: {
	 //		  writeLogString(fpnLog, "ERROR : User defined potential evapotranspiration data type is invalid.\n", 1, 1);
	 //		  return -1;
	 //	  }
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading user defined potential evapotranspiration data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
	 ...
	 //	  string fpnData = "";
	 //	  int returnv = 1;
	 //	  fpnData = snowpackTemp[order - 1].FilePath + "/" + snowpackTemp[order - 1].FileName;
	 //	  switch (prj.snowpackTempDataType)
	 //	  {
	 //	  case weatherDataType::Raster_ASC: {
	 //		  ascRasterFile* wdAsc;
	 //		  wdAsc = new ascRasterFile(fpnData);
	 //		  if (compareASCwithDomain(fpnData, "snow pack temperature", wdAsc->header, di.nCols, di.nRows, di.dx) == -1) {
	 //			  return -1;
	 //		  }
	 //		  omp_set_num_threads(prj.mdp);
	 //#pragma omp parallel for
	 ...
	 //	  }
	 //	  case weatherDataType::Mean: {
	 //		  double wdValue = stod(snowpackTemp[order - 1].value);
	 //#pragma omp parallel for schedule(guided)
	 //		  for (int i = 0; i < di.cellNnotNull; ++i) {
	 //			  // 유역의 전체 강우량은 inlet 등으로 toBeSimulated == -1 여도 계산에 포함한다.
	 //			  // 상류 cv 개수에 이 조건 추가하려면 주석 해제.
	 ...
	 //	  }
	 //	  default: {
	 //		  writeLogString(fpnLog, "ERROR : Snow pack temperature data type is invalid.\n", 1, 1);
	 //		  return -1;
	 //	  }
	 //	  }
	 //	  if (returnv == -1) {
	 //		  writeLogString(fpnLog, "ERROR : An error was occurred while reading snow pack temperature data.\n", 1, 1);
	 //		  writeLogString(fpnLog, "    Data file : " + fpnData + ".\n", 1, 1);
	 //	  }
	 //	  return returnv;
	 //  }
