    ...
    string strModel = "")
{
    ...
    tsrt.simDurationrRT_h = 24 * 100; // 충분히 100일 동안 모의하는 것으로 설정
...
void grmRealTime::runGRMRT()
{
    ...
    if (tsrt.g_strModel == "") {
        fpn_rfASC = rtef.fpRTRFfolder + "\\"
            + getYYYYMMfromYYYYMMddHHmm(rtef.rtstartingTime)
    ...
    if (prj.applyFC == 1) {
     for (int idx : fccds.cvidxsFCcell) {
        ...
        if (CONST_bUseDBMS_FOR_RealTimeSystem == true) {
            if (readDBandFillFCdataForRealTime(rtef.rtstartingTime) == -1) {
                return;
            }
        }
        else {
            if (readCSVandFillFCdataForRealTime(rtef.fpnRTFCdata,
                rtef.rtstartingTime) == -1) {
                return;
            }
        }
        for (int idx : fccds.cvidxsFCcell) {
            if (fccds.flowData_m3Ps[idx].size() < 1) {
                writeLog(fpnLog, "[" + prj.fcs[idx].fcName +
                    "] 지점에서 유출해석 시작 시간에서의 flow control 자료가 없습니다.", 1, 1);
                writeLog(fpnLog, "유출해석 시작시간과 댐방류량, inlet 자료 등 flow control 자료를 확인하시길 바랍니다.", 1, 1);
                return;
            }
        }
    ...
    writeLog(fpnLog, "Real time simulation was started. Predicion = "
        + to_string(tsrt.isPrediction), 1, 1);
    startSimulationRT();
}
