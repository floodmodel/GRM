...
Public Class cChart
    ...
    Public Sub New(ByVal pbChartMain As PictureBox, ByVal pbChartRF As PictureBox,
                   ...
                   ByVal inLegendsObs As List(Of String),
    ...
    Public Sub New(ByVal pbChartMain As PictureBox, ByVal pbChartRF As PictureBox,
               ByVal inQobs As Dictionary(Of Integer, List(Of Nullable(Of Double))),
               ByVal inObsLegends As List(Of String),
               ByVal inQsim As Dictionary(Of Integer, List(Of Nullable(Of Double))),
    ...
    Private Sub initializeVariables(ByVal pbChartMain As PictureBox, ByVal pbChartRF As PictureBox,
                                    ByVal inQobs As Dictionary(Of Integer, List(Of Nullable(Of Double))), ByVal inObsLegends As List(Of String), ByVal inSimLegends As List(Of String),
                                    Optional ByVal inQsims As Dictionary(Of Integer, List(Of Nullable(Of Double))) = Nothing,
                                    Optional ByVal inRF As Dictionary(Of Integer, Double) = Nothing, Optional ByVal inXLabels As Dictionary(Of Integer, String) = Nothing)
        ...
        mRFs = New Dictionary(Of Integer, Double)
        mQobss = New Dictionary(Of Integer, List(Of Nullable(Of Double)))
        mQsims = New Dictionary(Of Integer, List(Of Nullable(Of Double)))
    ...
    Public Function DrawChart() As Boolean
        Try
            ...
            For n As Integer = 0 To SeriesCountInAClassOBS - 1
                Dim valuesObs As New List(Of Nullable(Of Double))
            ...
            For m As Integer = 0 To mQsims.Count - 1 '키는 0부터 시작
                If mQsims.ContainsKey(m) = False Then
                    mQsims.Add(m, New List(Of Nullable(Of Double)))
                    For n As Integer = 0 To SeriesCountInAClassSIM - 1
                        mQsims(m).Add(Nothing)
                    Next n
                ElseIf mQsims(m) Is Nothing Then
                    mQsims(m) = New List(Of Nullable(Of Double))
                    For n As Integer = 0 To SeriesCountInAClassSIM - 1
                        mQsims(m).Add(Nothing)
                    Next n
                End If
            ...
            For n As Integer = 0 To SeriesCountInAClassSIM - 1
                Dim valuesSim As New List(Of Nullable(Of Double))
                Dim lst As New List(Of Double)
                For m As Integer = 0 To mQsims.Count - 1 '키는 0부터 시작
                    valuesSim.Add(mQsims(m)(n))
                Next
    ...
    Public Function AddQsimAndRFpointAndUpdateChart(ByVal RfToAdd As Single, ByVal QsimToAdd As List(Of Nullable(Of Double))) As Boolean
        mAddedValueCount += 1
        ...
        For n As Integer = 0 To SeriesCountInAClassSIM - 1
            Dim l As New List(Of Nullable(Of Double))
    ...
    Public Shared Function ReadObservedDataAndSetDictionary(ByVal obsFPN As String) As Dictionary(Of Integer, List(Of Nullable(Of Double)))
        Dim strLines() As String = System.IO.File.ReadAllLines(obsFPN)
        Dim qObs As New Dictionary(Of Integer, List(Of Nullable(Of Double)))
        For n As Integer = 1 To strLines.Length - 1 '0번은 레전드 이므로.. 스킵
            ...
            Dim v As Single
            Dim vAline As New List(Of Nullable(Of Double))
            'If LineParts IsNot Nothing  Then
            For Each strV As String In LineParts
                If Single.TryParse(strV, v) Then
                    vAline.Add(v)
                Else
                    MsgBox("Observed data is invalid.  " + vbCrLf +
                    "Check the observed data file.    " + vbCrLf +
